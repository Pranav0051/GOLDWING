        ...
        <div className="min-h-screen bg-[#0B0F19] text-white p-4 font-sans flex flex-col items-center">
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-lg bg-[#111827] border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl">
                ...
                <form onSubmit={handleSimulateScan} className="mb-6 relative">
                    <input
                        type="text"
                        placeholder="e.g. GW-847291"
                        value={scanData}
                        onChange={(e) => setScanData(e.target.value)}
                        className="w-full bg-white/5 border border-white/10 focus:border-blue-500/50 rounded-2xl pl-4 pr-12 py-4 text-center font-mono text-lg uppercase focus:outline-none transition-colors"
                        autoFocus
                    />
                ...
                {error && (
                    <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-red-500/10 border border-red-500/30 text-red-400 p-4 rounded-2xl flex items-start gap-3">
                ...
                {result && (
                    <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="space-y-4">
                        <div className={`p-5 rounded-2xl border ${result.status === 'Cancelled' ? 'bg-red-500/10 border-red-500/30' : result.status === 'Completed' ? 'bg-gray-800/50 border-gray-700' : 'bg-[#D4AF37]/10 border-[#D4AF37]/30'}`}>
                            <div className="flex justify-between items-start mb-4">
                                ...
                                {result.status === 'Cancelled' ? (
                                    <span className="bg-red-500/20 text-red-500 px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap"><XCircle className="w-4 h-4 inline mr-1" /> CANCELLED</span>
                                ) : result.status === 'Completed' ? (
                                    <span className="bg-gray-500/20 text-gray-400 px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap"><ShieldCheck className="w-4 h-4 inline mr-1" /> ATTENDED</span>
                                ) : (
                                    <span className="bg-green-500/20 text-green-400 px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap"><ShieldCheck className="w-4 h-4 inline mr-1" /> VALID ENTRY</span>
                                )}
                            </div>
                        ...
                        {result.status !== 'Cancelled' && result.status !== 'Completed' && (
                            <button onClick={handleAdmit} className="w-full py-4 bg-green-600 hover:bg-green-500 text-white font-bold rounded-xl flex justify-center items-center shadow-[0_0_20px_rgba(22,163,74,0.3)] transition-all">
